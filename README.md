...
This document will show you how to integrate the [TiDB Cloud](https://tidbcloud.com/) Serverless Tier Cluster and [Amazon QuickSight](https://aws.amazon.com/quicksight/) via [AWS PrivateLink](https://aws.amazon.com/privatelink/). In this document, we need to use the [AWS CloudFormation](https://aws.amazon.com/cloudformation/) and [AWS CLI](https://aws.amazon.com/cli/) to simplify the progress.
...
The outline prograss is:

1. `[Terminal]` Run the first script `1-create-private-link-with-ec2.sh`. Create the VPC (with the surrunding components), EC2 Interface, VPC Endpoint, etc.
2. `[Console]` Use the **VPC Endpoint ID** from first step to confirm in TiDB Cloud.
3. `[Console]` Use the **Security Group ID** and **Resolver Endpoint ID** Create the VPC connection ARN.
4. `[Terminal]` Run the second script `2-active-vpc-endpoint.sh` to active the **VPC Endpoint**.
5. `[Terminal]` Run the third script `3-create-quicksight.sh` to create the **QuickSight** components.
6. `[QuickSight]` Start to analysis in Amazon QuickSight.
...
- An AWS [Identity and Access Management (IAM) user](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_users.html) with the following requirements:
  ...
  - The user has the following permissions:
    ...
    - `AWSCloudFormationFullAccess`: This guidence use the **AWS CloudFormation** to create AWS resources.
    ...
    - The **Amazon QuickSight** don't have an AWS managed policy. So you need to [create a customer inline policy](https://uconsole.aws.amazon.com/iam/home#/policies$new?step=edit), and add this permission to this user:
...
- Import a dataset to analysis. In this document, use the Fitness Trackers Products Ecommerce for example. You can use the [fitness_trackers.sql](/data/fitness_trackers.sql) to create the table in TiDB.
...
2. Replace the `ParameterValue` of `TiDBVPCEndpointServiceName` to your TiDB Cloud vpc endpoint service name.
...
4. The end of output message will look like below. Please remeber the `VPC Endpoint ID`, `QuickSightSecurityGroupID` and `QuickSightResolverEndpointID` to use in next two steps:
  ...
  Those are the properties for create Amazon QuickSight:
...
> **Note:**
>
> If you want to know the actually effect of each resource, please click the components link get more information:
>
...
- Go to the page for [create a VPC Connection](https://quicksight.aws.amazon.com/sn/admin#vpc-connections).
- Click **VPC connection**.
- Select VPC named `quick-sight-vpc` and the only one subnet.
- Fill the `QuickSightSecurityGroupID`(from the [first step](#1-create-the-vpc-with-all-components--privatelink--ec2)) to `Security group ID`
- Fill the `QuickSightResolverEndpointID` (from the [first step](#1-create-the-vpc-with-all-components--privatelink--ec2))to `DNS resolver endpoints (optional)` .

  ![quicksight-vpc-connection](/assets/quicksight-vpc-connection.png)

- Click `Create`.
- Remeber the `VPC connection ARN` param to use it later.

  ![quicksight-vpc-connection-arn](/assets/quicksight-vpc-connection-arn.png)

## 4. Active the VPC Endpoint

You can active the VPC Endpoint after the TiDB Cloud confirmed.
...
This step will create a Amazon QuickSight datasource via VPC connection to TiDB. And use this datasource to create a dataset which is the table `fitness_trackers` we imported.
...
3. Run the script: `3-create-private-link-with-ec2.sh`.

  ```bash
  ./3-create-quicksight.sh
  ```

## 6. Start to analysis in Amazon QuickSight
